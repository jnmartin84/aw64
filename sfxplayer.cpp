...
static inline uint16_t S_READ_BE_UINT16(const void *ptr) {
...
void SfxPlayer::loadSfxModule(uint16_t resNum, uint16_t delay, uint8_t pos) {
	...
	if (me->state == MEMENTRY_STATE_LOADED && me->type == Resource::RT_MUSIC) {
		...
		_sfxMod.numOrder = *((uint8_t*)me->bufPtr + 0x3E);// *READ_BE_UINT16(me->bufPtr + 0x3E);
		...
		if (delay == 0) {
			_delay = S_READ_BE_UINT16(me->bufPtr);
		} else {
			_delay = delay;
		}
...
void SfxPlayer::prepareInstruments(const uint8_t *p) {
	...
	for (int i = 0; i < 15; ++i) {
		...
		if (resNum != 0) {
			ins->volume = /*(uint8_t)*/S_READ_BE_UINT16(p);
			MemEntry *me = &res->_memList[resNum];
...
void SfxPlayer::handlePattern(uint8_t channel, const uint8_t *data) {
	...
	pat.note_1 = S_READ_BE_UINT16(data + 0);
	pat.note_2 = S_READ_BE_UINT16(data + 2);
	if (pat.note_1 != 0xFFFD) {
		...
		if (sample != 0) {
			...
			if (ptr != 0) {
				...
				pat.sampleLen = S_READ_BE_UINT16(ptr) * 2;
				uint16_t loopLen = S_READ_BE_UINT16(ptr + 2) * 2;
				if (loopLen != 0) {
	...
	if (pat.note_1 == 0xFFFD) {
		...
		*_markVar = pat.note_2;
		...
		if (pat.note_1 == 0xFFFE) {
			...
			uint16_t freq = //3579546 / pat.note_1; 
			7159092 / (pat.note_1 * 2);
