...
#else
	vmVariables[0xF2] = /*(_res->getDataType() == Resource::DT_AMIGA || _res->getDataType() == Resource::DT_ATARI) ? 6000 :*/ 4000;
	// this variable are set by the engine executable
...
void VirtualMachine::op_condJmp() {
	...
	uint8_t varbyte = _scriptPtr.fetchByte();
	int16_t b = vmVariables[varbyte];
	...
	case 0:	// jz
		expr = (b == a);
		// bypass protection
		if (res->currentPartId == GAME_PART_FIRST) {
			//
			// 0CB8: jmpIf(VAR(0x29) == VAR(0x1E), @0CD3)
			// ...
			//
			if (varbyte == 0x29 && (opcode & 0x80) != 0) {
				// 4 symbols
				...
				vmVariables[0x32] = 6;
				vmVariables[0x64] = 20;
				//warning("VirtualMachine::op_condJmp() bypassing protection");
				expr = true;
			}
		}
		break;
